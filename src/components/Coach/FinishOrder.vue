<template>
  <div id="order-management">
    <ul class="order-status clear">
      <li @click="notFinishOrder()">未完成</li>
      <li @click="finishOrder()" class="active">已完成</li>
    </ul>
        ...
        <div class="order-header clear">
          ...
          <ul class="we-info">
            <li class="clear">
              ...
              <span class="right" v-if="item.status == 1">已支付</span>
              <span class="right" v-if="item.status == 2">已取消</span>
              <span class="right" v-if="item.status == 3">退款</span>
              <span class="right" v-if="item.status == 4">已完成</span>
            </li>
...
//        this.$router.push({path: '/Coach/FinishOrder'})
    ...
    created(){
      let self = this;
      this.$axios.get(this.ApiRoute.getcoachorder + '?type=1', {}, {
        headers: {
